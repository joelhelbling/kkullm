...
func TestRenameAgent_OK_BackfillsHistoricalComments(t *testing.T) {
	...
	if err := s.RenameAgent(agent.ID, "new-name"); err != nil {
		t.Fatalf("RenameAgent: %v", err)
	}

	var authorName string
	if err := s.db.QueryRow(
		"SELECT author_name FROM comments WHERE agent_id = ?",
		agent.ID,
...
func TestRenameAgent_DuplicateName(t *testing.T) {
	...
	if err := s.RenameAgent(a2.ID, "alpha"); err == nil {
		t.Fatalf("expected error renaming to duplicate name, got nil")
	}
...
func TestRenameAgent_EmptyName(t *testing.T) {
	s := setupTestDB(t)
	proj := createTestProject(t, s)
	agent := createTestAgent(t, s, "alpha", proj.ID)

	if err := s.RenameAgent(agent.ID, ""); err == nil {
